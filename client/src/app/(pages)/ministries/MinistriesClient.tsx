...
export default function MinistriesClient() {
  useGSAPAnimation(() => {
    ...
    gsap.set([".page-header", ".ministry-card", ".cta-section"], { 
      opacity: 0,
      y: function(index: number, element: Element) {
        if (element.classList.contains("page-header")) return -50;
        if (element.classList.contains("ministry-card")) return 50;
    ...
    tl.to(".ministry-card", { opacity: 1, y: 0, stagger: 0.15, duration: 0.7 }, "-=0.5");
